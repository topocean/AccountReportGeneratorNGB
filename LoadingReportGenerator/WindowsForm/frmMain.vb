...
Public Class frmMain
    ...
    Private Sub Timer1_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Timer1.Tick
        ...
        Dim rptParaNameStr, rptParaValueStr As String
        ...
        If count = My.Settings.TimeInterval Then
            ...
            Try
                ...
                If AutoPrintQuery = 0 And Not My.Settings.IsAccDisable Then
            ...
            Try
                ' Get the retry email not sent query from last 8 hour
                sql = "EXEC usp_GetRptQueryRetry " & My.Settings.GenID & ""
                ...
                If sql <> "" Then
                    ...
                    With ds.Tables(0).Rows(0)
                        ...
                        RptID = common.NullVal(.Item("RptID").ToString, "")
                        RptName = common.NullVal(.Item("RptName").ToString, "")
                        RptDate = Format(CType(.Item("RptDate").ToString, Date), "yyyy-MM-dd")
                        rptFile = common.NullVal(.Item("Url").ToString, "")
                        sPID = common.NullVal(.Item("sPID").ToString, "")
                        sUID = common.NullVal(.Item("sUID").ToString, "")
                        AutoJob = common.NullVal(.Item("AutoJob"), 0)
                        AutoEmail = common.NullVal(.Item("AutoEmail").ToString, "")
                        ...
                        Else
                            usrDtl = Split(common.NullVal(.Item("Usr").ToString, ""), "|")
                        End If
                        isEmail = common.NullVal(.Item("Email").ToString, 0)
                        exportReport = common.NullVal(.Item("RptType").ToString, "")
                    End With
                    ...
                    If UID <> "" Then
                        If PrevsUID <> sUID Then
                            If PrevsUID <> "" Then
                                ...
                                clsMail.MailReport(PrevsUID, PrevUsrDtl, PrevsUID & ".zip", PrevRptName, PrevParaNames, PrevParaValues, 0, True, RptDate)
                        ...
                        If isEmail = 1 Then
                            common.SaveLog("Prepare retry sending email to requested user. (UID: " & UID & ")")
                            Me.lstDisplay.Items.Add(Format(Now, "yyyy.MM.dd HH:mm:ss") & " - Prepare retry sending email to requested user.")
                            Me.lstDisplay.SelectedIndex = Me.lstDisplay.Items.Count - 1

                            Dim fileExt As String = ""

                            If rptFile <> "" Then
                                Dim fileInfo As New IO.FileInfo(rptFile)
                            ...
                            clsMail.MailReport(UID, usrDtl, rptFile, RptName, paraNames, paraValues, isNoData, True, RptDate)
            ...
            Catch ex As Exception
                If UID <> "" Then
                    ' Update Query Status
                    sql = "EXEC usp_PrintJob_Fail '" & UID & "', '" & common.setQuote(ex.Message) & "'"
                    cmd.CommandText = sql
                    cmd.CommandTimeout = My.Settings.Timeout
                    cmd.ExecuteNonQuery()
                End If
